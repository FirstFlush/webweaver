...
from webweaver.config import REQUEST_WAIT_BASE, REQUEST_WAIT_MAX
from webweaver.exceptions import RetryAfterHeaderMalformed, SpiderRetryTimeout
    ...
    async def handle_response(self):
        ...
        if wait_time:
            if wait_time > REQUEST_WAIT_MAX:
                self.spider_interface.log(SpiderRetryTimeout(f"Wait time of {wait_time} seconds is too long"))
    ...
    def wait_time(self) -> int:
        ...
        if retry_header:
            try:
                ...
                self.spider_interface.log(RetryAfterHeaderMalformed(f"Retry-After header: {retry_header}"))
        if wait_time == 0:
